...
class StateTracker:
    ...
    def apply_draw(self, actor: int, tile: Optional[Tile] = None):
        ...
        self.state.current_turn = actor  # after draw, same actor will usually discard
    ...
    def apply_call(self, actor: int, kind: str, tiles: List[Tile], from_who: Optional[int]):
        meld = Meld(kind=kind, tiles=[normalize_tile(x) for x in tiles], from_who=from_who)
        self.state.players[actor].open_melds.append(meld)
        if kind in {"KAN", "ADDED_KAN", "CONCEALED_KAN"}:
            self.state.wall.kan_count += 1
        # When someone calls, they usually become the next discarder (simple rule)
    ...
    def apply_riichi(self, actor: int):
        ...
        # Actor will still discard; leave current_turn as actor if not already
        self.state.current_turn = actor
    ...
    def can_pon(self, discard_tile: Tile) -> bool:
        if self.state.our_seat == self.state.current_turn:
            return False
        hand = self.state.players[self.state.our_seat].concealed
        return hand.count(normalize_tile(discard_tile)) >= 2

    def can_chi(self, discarder: int, discard_tile: Tile) -> bool:
        # Chi only allowed by the LEFT player of the discarder
        left = self.next_seat(discarder)
        if self.state.our_seat != left:
            return False
        t = normalize_tile(discard_tile)
        if t.endswith('z'):
            return False
        rank = int(t[:-1])
        suit = t[-1]
        hand = self.state.players[self.state.our_seat].concealed
        needed_sets = [
            [f"{rank-2}{suit}", f"{rank-1}{suit}"] if 3 <= rank <= 9 else None,
            [f"{rank-1}{suit}", f"{rank+1}{suit}"] if 2 <= rank <= 8 else None,
            [f"{rank+1}{suit}", f"{rank+2}{suit}"] if 1 <= rank <= 7 else None,
        ]
        for need in needed_sets:
            if not need:
                continue
            if all(hand.count(n) >= 1 for n in need):
                return True
        return False

    def can_kan_from_discard(self, discard_tile: Tile) -> bool:
        hand = self.state.players[self.state.our_seat].concealed
        return hand.count(normalize_tile(discard_tile)) == 3
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Riichi Tracker (Mahjong Soul) — v0.2")
    ...
    # ---- Tile button panel ----
    def make_tile_panel(self, parent: tk.Widget, on_click):
        ...
        for gi, (label, tiles) in enumerate(SUIT_GROUPS):
            ...
            for t in tiles:
                ...
                col += 1
            # wrap not needed due to few buttons per group
        return outer
    ...
    def apply_operation(self):
        ...
        try:
            if op == "DRAW":
                ...
                self.last_discard = (actor, tiles[0])
                # On opponent discard, check if we (our seat) can call → "AI decision point" for calling
                if actor != st.our_seat:
                    self._maybe_call_prompt(actor, tiles[0])
            elif op in {"CHI","PON","KAN","ADDED_KAN","CONCEALED_KAN"}:
                if not tiles:
                    raise ValueError(f"{op} needs tiles")
                # For from_who, if from last_discard, infer
                from_who = self.last_discard[0] if self.last_discard else None
                tr.apply_call(actor, op, tiles, from_who)
                self.last_discard = None
            elif op == "RIICHI":
                ...
                # Placeholder: just record conceptually by updating turn or doing nothing
                if op == "PASS" and self.last_discard is not None:
                    # If everyone passes after a discard, turn moves on already by discard logic
                    pass
                # Ron/Tsumo would end the hand; we simply note and freeze turn
            else:
        ...
        # If it's now our turn, trigger AI placeholder
